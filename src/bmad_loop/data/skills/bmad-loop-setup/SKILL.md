...
Both scripts output JSON to stdout with results. If either exits non-zero, surface the error and stop — a non-zero exit always means the config was left as it was (each write is verified first and committed atomically, so nothing is ever half-written), so it is safe to fix the reported cause and re-run: both merges are idempotent. Two errors are worth relaying verbatim because only the user can clear them: an existing `custom/config.toml` that is **not valid TOML** (bmad-loop will not edit a config it cannot parse), and a Python older than **3.11** on the TOML layout. The scripts automatically read legacy config values as fallback defaults. Legacy config files are **preserved, never deleted** — on BMAD v6 the per-module and core `config.yaml` / `module-help.csv` are live, manifest-tracked files (so `legacy_configs_deleted` / `legacy_csvs_deleted` are always empty). On the TOML layout, inert pre-6.10 files directly under `_bmad/` are likewise only **reported**, in `orphans_detected`.
