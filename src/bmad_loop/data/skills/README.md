...
| `bmad-loop-setup`   | — (scaffolded)       | registers the module in `_bmad/` config + help — `custom/config.toml` + `_config/bmad-help.csv` on BMAD v6.10+, legacy `config.yaml` + `module-help.csv` before — **installs the orchestrator tool from Git**, runs `bmad-loop init` + `validate` |
