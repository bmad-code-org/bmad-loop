...
- **The detector itself gets executable coverage.** The two newest detectors — env-read and
  git-argv — carry probe matrices: the scan is split (`_scan_source`) so probe fixtures run
