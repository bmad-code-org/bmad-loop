...
Like `spec_checkpoint` and `done_checkpoint`, this is a **human-authored, caller-only field**: you add it at breakdown time with the ledger open. Upstream Story Breakdown does not emit it yet ([BMAD-METHOD#2619](https://github.com/bmad-code-org/BMAD-METHOD/issues/2619)), and re-deriving `stories.yaml` rewrites the file — so record the intent in `.memlog.md` alongside the story, or the declaration is lost on the next re-derive.
